...
# Location: Linux_Dist/tools/eac3.py
...
def worker_eac3(slot_id):
    while True:
        ...
        # FFMPEG expects 'k' suffix for audio bitrate usually
            ...
            while True:
                ...
                if chunk:
                    ...
                    if match:
                        t, b, s = match.groups()
                        slot_status[slot_id] = (
                            f"{slot_id + 1}: [EAC3] {fname}.. T:{t} Spd:{s} ({channels}ch)"
                        )
        except Exception as e:
...
def mux_final_files():
    ...
    for mkv_path in current_dir.glob("*.mkv"):
        ...
        # Matches:  Video_track1_eng.eac3  OR  Video_track1_eng.thd
        pattern = re.compile(
        ...
        for tid in sorted_ids:
            ...
            # Logic: Prefer EAC3, fallback to ORIGINAL
            ...
            is_eac3 = final_path.suffix == ".eac3"
...
def main():
    try:
        ...
        # Filter: Remove files that are already .eac3 or should be ignored
        to_eac3 = []
        ...
        if to_eac3:
            # No FLAC intermediary needed for ffmpeg->eac3
